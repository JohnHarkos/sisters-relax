...
exports.handler = async function(event) {
  ...
  try {
    ...
    const { events } = body;
    // Code de test : permet de voir les évènements serveur dans l'outil
    // « Évènements de test » de Meta. Uniquement présent si le site l'envoie.
    const testCode = typeof body.test_event_code === 'string'
      ? body.test_event_code.trim().slice(0, 40) : null;
    ...
    const response = await fetch(
      `https://graph.facebook.com/v21.0/${PIXEL_ID}/events?access_token=${TOKEN}`,
      {
        ...
        body: JSON.stringify(
          testCode ? { data: metaEvents, test_event_code: testCode }
                   : { data: metaEvents })
      }
    );
