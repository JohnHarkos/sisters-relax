// ============================================================
// API CONVERSIONS META — Canal serveur (la "fibre")
...
//   META_PIXEL_ID   = ID du pixel Sister Relax
...
exports.handler = async function(event) {
  ...
  try {
    ...
    // Construction des événements pour Meta CAPI
    const metaEvents = events.map(evt => {
      ...
      // Données utilisateur hashées (Advanced Matching serveur)
      if (evt.user_data) {
      ...
      // Données personnalisées (prestation, prix)
      if (evt.custom_data) {
